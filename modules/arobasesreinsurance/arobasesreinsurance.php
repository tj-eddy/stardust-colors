...
include_once _PS_MODULE_DIR_ . 'arobasesreinsurance/arobasesreinsuranceClass.php';

class Arobasesreinsurance extends Module {

    public function __construct() {
        $this->name = 'arobasesreinsurance';
        ...
        $this->displayName = $this->l('Reassurance arobases');
        $this->description = $this->l('Bloque pour reassurance arobases.');
    ...
    public function install() {
        return parent::install() &&
                $this->installDB() &&
                Configuration::updateValue('AROBASESREINSURANCE_NBBLOCKS', 5) &&
    ...
    public function installDB() {
        ...
        $return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'arobasesreinsurance` (
				`id_arobasesreinsurance` INT UNSIGNED NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL ,
				`file_name` VARCHAR(100) NOT NULL,
                `new_window` TINYINT DEFAULT 0 NULL,
				PRIMARY KEY (`id_arobasesreinsurance`)
			) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8 ;');

        $return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'arobasesreinsurance_lang` (
				`id_arobasesreinsurance` INT UNSIGNED NOT NULL AUTO_INCREMENT,
				`id_lang` int(10) unsigned NOT NULL ,
				`text` VARCHAR(300) NOT NULL,
				`lien` VARCHAR(300) NOT NULL,
				PRIMARY KEY (`id_arobasesreinsurance`, `id_lang`)
			) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8 ;');
    ...
    public function uninstall() {
        ...
        return Configuration::deleteByName('AROBASESREINSURANCE_NBBLOCKS') &&
                $this->uninstallDB() &&
                parent::uninstall();
    }

    public function uninstallDB() {
        return Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'arobasesreinsurance`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'arobasesreinsurance_lang`');
    }
        ...
        if (isset($_POST['nbblocks'])) {
            for ($i = 1; $i <= (int) $_POST['nbblocks']; $i++) {
                ...
                Db::getInstance()->execute('
					INSERT INTO `' . _DB_PREFIX_ . 'arobasesreinsurance` (`filename`,`text`,`lien`)
					VALUES (
					"' . ((isset($filename[0]) && $filename[0] != '') ? pSQL($filename[0]) : '') .
    ...
    public function removeFromDB() {
        ...
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'arobasesreinsurance`');
    }

    public function getContent() {
        $html = '';
        $id_reinsurance = (int) Tools::getValue('id_arobasesreinsurance');
        if (Tools::isSubmit('statusarobasesreinsurance') && (isset($_GET['id_arobasesreinsurance']))) {
            $reinsurance = new arobasesreinsuranceClass((int) $id_reinsurance);
            $reinsurance->new_window = $reinsurance->new_window == 0 ? 1 : 0;
            
            //$reinsurance->hydrate($_GET);
            $reinsurance->save();
            $helper = $this->initList();
            return $html . $helper->generateList($this->getListContent((int) Configuration::get('PS_LANG_DEFAULT')), $this->fields_list);
        }
        if (Tools::isSubmit('savearobasesreinsurance')) {
            if ($id_reinsurance = Tools::getValue('id_arobasesreinsurance'))
                $reinsurance = new arobasesreinsuranceClass((int) $id_reinsurance);
            else
                $reinsurance = new arobasesreinsuranceClass();
            ...
            if ($reinsurance->validateFields(false) && $reinsurance->validateFieldsLang(false)) {
                ...
                $this->_clearCache('arobasesreinsurance.tpl');
            } else
                $html .= '<div class="conf error">' . $this->l('An error occurred while attempting to save.') . '</div>';
        }

        if (Tools::isSubmit('updatearobasesreinsurance') || Tools::isSubmit('addarobasesreinsurance')) {
            ...
            if ($id_reinsurance = Tools::getValue('id_arobasesreinsurance')) {
                $this->fields_form[0]['form']['input'][] = array('type' => 'hidden', 'name' => 'id_arobasesreinsurance');
                $helper->fields_value['id_arobasesreinsurance'] = (int) $id_reinsurance;
            }
                if ($id_reinsurance) {
                    $reinsurance = new arobasesreinsuranceClass((int) $id_reinsurance);
                    $helper->fields_value['new_window'] = $reinsurance->new_window;
                }
                else {
                    $helper->fields_value['new_window'] = 0;
                }
            
            return $html . $helper->generateForm($this->fields_form);
        } else if (Tools::isSubmit('deletearobasesreinsurance')) {
            $reinsurance = new arobasesreinsuranceClass((int) $id_reinsurance);
        ...
        if (isset($_POST['submitModule'])) {
            Configuration::updateValue('AROBASESREINSURANCE_NBBLOCKS', ((isset($_POST['nbblocks']) && $_POST['nbblocks'] != '') ? (int) $_POST['nbblocks'] : ''));
    ...
    protected function getListContent($id_lang) {
        return Db::getInstance()->executeS('
			SELECT r.`id_arobasesreinsurance`, r.`id_shop`, r.`file_name`, rl.`text`, rl.`lien`, r.`new_window`
			FROM `' . _DB_PREFIX_ . 'arobasesreinsurance` r
			LEFT JOIN `' . _DB_PREFIX_ . 'arobasesreinsurance_lang` rl ON (r.`id_arobasesreinsurance` = rl.`id_arobasesreinsurance`)
    ...
    protected function initForm() {
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'hint' => $this->l('Ouvrir dans un nouvel onglet.')
                ),
                /*array(
                    'type'      => 'radio',
                    'label'     => $this->l('Ouvrir dans un nouvel onglet.'),
                    'name'      => 'new_window',                              // The content of the 'id' attribute of the <input> tag.
                    'required'  => true,
                    'class'     => 't',
        ...
        $helper->module = $this;
        $helper->name_controller = 'arobasesreinsurance';
        ...
        $helper->submit_action = 'savearobasesreinsurance';
    ...
    protected function initList() {
        ...
        $arrayResult = Db::getInstance()->executeS(
            'SELECT * FROM '._DB_PREFIX_.'arobasesreinsurance '
        );
        //$helper->listTotal(count($arrayResult));
        
        $helper->shopLinkType = '';
        $helper->simple_header = false;
        $helper->identifier = 'id_arobasesreinsurance';
    ...
    public function hookFooter($params) {
        ...
        if (!$this->isCached('arobasesreinsurance.tpl', $this->getCacheId())) {
            $infos = $this->getListContent($this->context->language->id);
            $this->context->smarty->assign(array('infos' => $infos, 'nbblocks' => count($infos)));
        }
        return $this->display(__FILE__, 'arobasesreinsurance.tpl', $this->getCacheId());
    }
    ...
    public function installFixtures() {
        $return = true;
        $tab_texts = array(
            array('text' => $this->l('Money back guarantee.'), 'file_name' => 'reinsurance-1-1.png'),
            array('text' => $this->l('In-store exchange.'), 'file_name' => 'reinsurance-2-1.png'),
            ...
            array('text' => $this->l('100% secure payment processing.'), 'file_name' => 'reinsurance-5-1.png'),
            array('text' => $this->l('Payment upon shipment.'), 'file_name' => 'reinsurance-3-1.png'),
            array('text' => $this->l('Free Shipping.'), 'file_name' => 'reinsurance-4-1.png'),
        );
