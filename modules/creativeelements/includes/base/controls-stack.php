...
abstract class ControlsStack extends BaseObject
{
     ...
     *
     *     @var string $type     injection type, either `control` or `section`
     *                            Default is `control`
     *     @var string $at       Where to inject. If `$type` is `control` accepts
     *                            `before` and `after`. If `$type` is `section`
     *                            accepts `start` and `end`. Default values based on
     *                            the `type`.
     *     @var string $of       Control/Section ID
     *     @var array  $fallback Fallback injection position. When the position is
    ...
    final public function getConfig()
    {
        if (null === $this->config) {
            ...
            if (method_exists($this, '_getInitialConfig')) {
    ...
    public function startControlsSection($section_id, array $args = [])
    {
        ...
        if (null !== $this->current_section) {
            exit(sprintf('Elementor: You can\'t start a section before the end of the previous section "%s".', $this->current_section['section'])); // XSS ok.
        }
    ...
    public function printTemplate()
    {
        ...
        if (method_exists($this, '_contentTemplate')) {
     ...
     *
     * @since 1.4.0
     * @deprecated 2.9.0 use `getInitialConfig()` instead
     *
     * @return array The initial config
     */
    protected function _getInitialConfig()
    {
        // _deprecated_function( __METHOD__, '2.9.0', 'getInitialConfig' );
     ...
     *
     * @since 2.0.0
     * @deprecated 2.9.0 use `contentTemplate()` instead
     */
    protected function _contentTemplate()
    {
        // _deprecated_function( __METHOD__, '2.9.0', 'contentTemplate' );
