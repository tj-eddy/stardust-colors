{if isset($groups)}
    ...
    {foreach $groups as $group}
        ...
        {assign var='lazy_loading' value=0}
        {if isset($group.lazy_loading) && $group.lazy_loading}{$lazy_loading=1}{/if}
                ...
                {if isset($group['banners']) && count($group['banners'])}
                    <div class="row block_content">
                        <div id="banner_box_{$group['id_st_banner_group']}" class="col-lg-12 banner_col" data-height="100">
                            {include file="module:stbanner/views/templates/hook/stbanner-block.tpl" banner_data=$group['banners'][0] banner_height=$group['height'] banner_style=$style banner_lazy_loading=$lazy_loading}
                        </div>
                    </div>
                {elseif isset($group['columns'])}
                    {include file="module:stbanner/views/templates/hook/stbanner-column.tpl" columns_data=$group['columns'] banner_style=$style banner_lazy_loading=$lazy_loading}
                {/if}
            </div>
