...
{if isset($blog)}
	{if $blog.id}
        <div id="blog_primary_block" class="blog_detail_{$blog.id}" itemscope itemtype="http://schema.org/NewsArticle" itemid="{$link->getModuleLink('stblog','article',['id_st_blog'=>$blog.id,'rewrite'=>$blog.link_rewrite])}">
            <meta itemscope itemprop="mainEntityOfPage"  itemType="https://schema.org/WebPage" itemid="{$link->getModuleLink('stblog','article',['id_st_blog'=>$blog.id,'rewrite'=>$blog.link_rewrite])}"/>
