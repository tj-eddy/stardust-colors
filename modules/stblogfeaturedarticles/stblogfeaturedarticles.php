...
class StBlogFeaturedArticles extends BaseSlider
{
    ...
    public static function displayShowOn($value, $row)
    {
        ...
        return Module::isInstalled('jscomposer') && Module::isEnabled('jscomposer') ? $context->getTranslator()->trans('For visual composer module', array(), 'Admin.Theme.Panda') : '--';
    }
    ...
    {        
        ...
        else
        {
            ...
            foreach($result as $k=>&$v)
            {
                ...
                if ($this->type == 2) {
                    $v['link'] = '';
                    $v['name'] = $this->getTranslator()->trans('Recent articles', array(), 'Shop.Theme.Panda');
                    // $v['description' ] = '';
                    ...
                    $v['name'] = $category->is_root_category ? $this->getTranslator()->trans('Featured articles', array(), 'Shop.Theme.Panda') : $category->name;
    ...
    function vc_map_init()
    {
        ...
        if($result)
            foreach($result AS $v) {
                if ($this->type == 2) {
                    $name = $this->getTranslator()->trans('Recent articles', array(), 'Shop.Theme.Panda');
                } else {
                    $category = new StBlogCategory((int)$v['id_st_blog_category'], (int)$this->context->language->id);
                    $name = $category->name;
                }
                $sliders[$this->getTranslator()->trans('ID:', array(), 'Admin.Theme.Panda').$v['id_st_blog_slider'].' -- '.$name] = $v['id_st_blog_slider'];
