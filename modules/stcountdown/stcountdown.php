...
class StCountDown extends Module
{
    ...
    public function getContent()
	{
        ...
        $this->_html .= '<script type="text/javascript">var systemFonts = \''.implode(',',$this->systemFonts).'\'; var googleFontsString=\''.json_encode($this->googleFonts).'\';</script>';
    ...
    public function hookDisplayHeader($params)
    {
        ...
        if (!$this->isCached('module:stcountdown/views/templates/hook/header.tpl', $cache_id))
        {
            ...
            $this->smarty->assign(array(
                ...
                's_countdown_id_products' => json_encode($id_products),
            ...
            if ($height = Configuration::get($this->_prefix_st.'HEIGHT'))
            {
                ...
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div{padding-top:'.$padding.'px;padding-bottom:'.$padding.'px;}.countdown_timer.countdown_style_0 .countdown_wrap_inner div span{height:'.$height_span.'px;line-height:'.$height_span.'px;}.countdown_timer.countdown_style_1 div, .countdown_wrap .countdown_title{height:'.$height.'px;line-height:'.$height.'px;}';
            }
            elseif($padding = (int)Configuration::get($this->_prefix_st.'PADDING'))
            {
                $height_span = floor((66-2*$padding)/2);
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div{padding-top:'.$padding.'px;padding-bottom:'.$padding.'px;}.countdown_timer.countdown_style_0 .countdown_wrap_inner div span{height:'.$height_span.'px;line-height:'.$height_span.'px;}';
            }
            ...
            if ($number_size = Configuration::get($this->_prefix_st.'NUMBER_SIZE'))
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div span.countdown_number, .countdown_timer.countdown_style_1 div, .pro_second_box .countdown_box{font-size:'.$number_size.'px;}';
            if ($font_size = Configuration::get($this->_prefix_st.'FONT_SIZE'))
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div span.countdown_text, .countdown_wrap .countdown_title{font-size:'.$font_size.'px;}';
            ...
            if (!Configuration::get($this->_prefix_st.'DIVIDER'))
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div{border-right:none;}';
            if ($divider_color = Configuration::get($this->_prefix_st.'DIVIDER_COLOR'))
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div{border-right-color:'.$divider_color.';}';
            //
            ...
            if($fontNumber)
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div span.countdown_number, .countdown_timer.countdown_style_1 div, .pro_second_box .countdown_box{'.($fontNumber != $this->_font_inherit ? 'font-family: "'.$fontNumber.'";' : '').$fontTextWeight.$fontTextStyle.'}';
            if($fontText)
                $custom_css .= '.countdown_timer.countdown_style_0 .countdown_wrap_inner div span.countdown_text, .countdown_wrap .countdown_title{'.($fontText != $this->_font_inherit ? 'font-family: "'.$fontNumber.'";' : '').$fontTextWeight.$fontTextStyle.'}';
            if($fontProText)
