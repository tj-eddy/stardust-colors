    ...
    {foreach $columns as $column}
		...
		<div id="steasy_column_{$column['id_st_easy_content_column']}" class="row {if $column['hide_on_mobile'] == 1} hidden-md-down {elseif $column['hide_on_mobile'] == 2} hidden-lg-up {/if}">
			{foreach $column['columns'] as $sub_column }
	            ...
	            <div id="steasy_column_{$sub_column['id_st_easy_content_column']}" class="{if $column['id_st_easy_content_column'] == 26 && $sub_column@first}col-lg-3{else}col-lg-{$column_width|replace:'.':'-'}{/if} col-sm-{$column_width_md|replace:'.':'-'} col-{$column_width_xs|replace:'.':'-'} steasy_column {if $sub_column['hide_on_mobile'] == 1} hidden-md-down {elseif $sub_column['hide_on_mobile'] == 2} hidden-lg-up {/if}" >
				...
				{if $column['id_st_easy_content_column'] == 26 && !$sub_column@last}
					<div class="vl_footer_{$sub_column['id_st_easy_content_column']}"></div>
				{/if}
	        {/foreach}
		</div>
        {/if}   
