...
{foreach $sub_column['elements'] as $element}
	<li class="{if $element.st_el_hide_on_mobile == 1} hidden-md-down {elseif $element.st_el_hide_on_mobile == 2} hidden-lg-up {/if}">
		<a href="{if isset($element.st_el_url) && $element.st_el_url}{$element.st_el_url}{else}#{/if}" id="stsocial_item_{$element.id_st_easy_content_element}" rel="nofollow" {if isset($element.st_el_header)}title="{$element.st_el_header}"{/if} {if isset($sub_column.st_new_window) && $sub_column.st_new_window} target="_blank" {/if} class="flex_box">
			<div class="social_wrap"><i class="{$element.st_el_icon}"></i></div>
			<div class="social_header flex_child">{if isset($element.st_el_header)}{$element.st_el_header}{/if}</div>
		</a>
	</li>
{/foreach}
</ul>
