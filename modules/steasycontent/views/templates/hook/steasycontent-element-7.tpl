...
function initMap_{/literal}{$sub_column.id_st_easy_content_column}{literal}()
{
    ...
    {foreach $sub_column['elements'] as $element}
        ...
        var marker_{$element.id_st_easy_content_element}{literal} = new google.maps.Marker({
            ...
            {if isset($element.st_gmap_marker_title) && $element.st_gmap_marker_title}title : '{$element.st_gmap_marker_title}',{/if}
            {literal}map: map
        });{/literal}
            
        // Infor window
        {if isset($element.st_gmap_marker_text) && $element.st_gmap_marker_text}
