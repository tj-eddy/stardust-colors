	...
	<li class="mo_ml_level_0 mo_ml_column">
		...
		{foreach from=$languages key=k item=language}
			...
			<li class="mo_ml_level_1 mo_sub_li">
				<a href="{url entity='language' id=$language.id_lang}" title="{$language.name_simple}" rel="alternate" hreflang="{$language.iso_code}" class="mo_ma_level_1 mo_sub_a">
				    {if $display_flags!=1}<img src="{$urls.img_lang_url}{$language.id_lang}.jpg" alt="{$language.iso_code}" width="16" height="11" class="mar_r4" />{/if}{$language.name_simple}
				</a>
			</li>
			{/if}
		{/foreach}
