...
class StMegaBrandClass
{
	...
	{
        $has_id_shop = Db::getInstance()->executeS('DESC `'._DB_PREFIX_.'manufacturer_lang` `id_shop`');
		$sql = 'SELECT m.*, ml.`description`, ml.`short_description`
			...
			LEFT JOIN `'._DB_PREFIX_.'manufacturer_lang` ml ON (
                ...
                '.($has_id_shop ? 'AND ml.`id_shop` = ' . (int)Context::getContext()->shop->id : '').'
			)
    ...
    public static function getMenuBrands($id_lang, $id_st_mega_menu)
    {
		...
		for ($i = 0; $i < $total_manufacturers; $i++)
        {
			...
			$manufacturers[$i]['image'] = Context::getContext()->link->getManufacturerImageLink($manufacturers[$i]['id_manufacturer'], 'brand_default');
            $manufacturers[$i]['image_2x'] = Context::getContext()->link->getManufacturerImageLink($manufacturers[$i]['id_manufacturer'], 'brand_default_2x');
        }
		return $manufacturers;
    }
}