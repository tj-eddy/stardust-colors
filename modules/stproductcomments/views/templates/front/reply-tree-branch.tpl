...
<div id="pcomment-{$reply.id_st_product_comment}" class="line_item">
    <div class="mb-2"><span class="font-weight-bold">{if $reply.is_admin}<i class="fto-user mr-1"></i>{/if}{if $reply.customer_name}{$reply.customer_name}{else}{l s='Someone' d='Shop.Theme.Panda'}-{$reply.id_st_product_comment}{/if}: </span><span class="pc_reply_body">{$reply.content|nl2br nofilter}</span></div>
    <div class="flex_box flex_space_between mb-2 pcomment-for-reply pb-2 general_bottom_border">
        ...
        <a href="javascript:;" class="btn_product_comment_reply" data-id="{$reply.id_st_product_comment}" title="{l s='Reply' d='Shop.Theme.Panda'}">{l s='Reply' d='Shop.Theme.Panda'}</a>
    </div>
