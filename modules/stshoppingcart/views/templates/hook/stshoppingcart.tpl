...
<div class="blockcart dropdown_wrap top_bar_item shopping_cart_style_{$block_cart_style} {if $block_cart_style==4} header_icon_btn_3 {/if} clearfix"
     data-refresh-url="{$refresh_url}">{strip}
    <a href="{$cart_url}" title="{l s='View my shopping cart' d='Shop.Theme.Panda'}" rel="nofollow"
       class="st_shopping_cart dropdown_tri header_item {if $click_on_header_cart} rightbar_tri {/if}"
       data-name="side_products_cart" data-direction="open_bar_{if $click_on_header_cart==2}left{else}right{/if}">
