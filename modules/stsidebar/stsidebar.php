...
class StSidebar extends Module implements WidgetInterface
{
    ...
    private function initNativeModules()
    {
        $this->_native_modules = array(
            ...
            7 => array('name' => $this->getTranslator()->trans('displayMobileNav - (My account infomation, language switcher, search box, megamenu can be here too)', array(), 'Modules.Stsidebar.Admin'), 'module' => ''),
            ...
            20 => array('name' => $this->getTranslator()->trans('Easy filter module', array(), 'Modules.Stsidebar.Admin'), 'module' => ''),
        );
    }
    ...
    public function hookDisplayRightBar($params)
    {
        ...
        foreach ($sidebar_items as $k=>$v) {
            if($v['native_modules'] != 9 && (in_array($v['native_modules'], $sidebar_items_right) || in_array($v['native_modules'], $sidebar_items_left)))
                continue;
            if($v['native_modules']==20 && (!isset($this->context->smarty->tpl_vars['listing']->value['rendered_facets']) || $this->context->smarty->tpl_vars['listing']->value['rendered_facets']=='')){
                continue;
            }
            if ($v['native_modules']) {
                if($v['location']==1)
                    $sidebar_items_left[] = $v;
                elseif($v['location']==0)
                    $sidebar_items_right[] = $v;
            } else {
                if($v['location']==1)
    ...
    public function _prepare_bar($bar){
        ...
        if($this->keyInBar('8', $bar) && $quick_search_mobile = Configuration::get('ST_QUICK_SEARCH_MOBILE'))
        {
    ...
    private function keyInBar($key, $bar = [])
    {
        foreach($bar as $val) {
            if ($val['native_modules'] == $key) {
                return true;
            }
        }
        return false;
