...
<div class="alert alert-info" role="alert">
    ...
    <p class="alert-text">{l s='Click a sticker to add it on on the product, click it once more to remove it from the product .' d='Admin.Theme.Panda'}</p>
</div>
    ...
    <div id="product-images-dropzone-ststickers" class="panel dropzone ui-sortable col-md-12 dz-started" data-max-size="8">
        ...
        <div class="dz-preview dz-image-preview" data-id_st_sticker="{$sticker.id_st_sticker}" data-id_st_sticker_map="{$sticker.id_st_sticker_map}" >
            <div class="dz-details">
                {if $sticker.name}
                    <div>{l s='Name:' d='Admin.Theme.Panda'}{$sticker.name}</div>
                {else}
                    <div>{l s='Id:' d='Admin.Theme.Panda'}{$sticker.id_st_sticker}</div>
                {/if}
                {if $sticker.text}<div>{l s='Text:' d='Admin.Theme.Panda'}{$sticker.text}</div>{/if}
            </div>
            <div class="dz-image bg" style="{if $sticker.image_multi_lang}background-image: url('{$sticker.image_multi_lang}');{/if}"></div>
            <div class="ishover {if !$sticker['id_st_sticker_map']} hide{/if}">{l s='Selected' d='Admin.Theme.Panda'}</div>
        </div>
        {/foreach}
    </div>
</div>
