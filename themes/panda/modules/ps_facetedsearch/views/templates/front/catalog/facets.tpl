 ...
 *}
{nocache}{strip}
{if is_array($displayedFacets) && count($displayedFacets)}
          ...
          {if $filter_position==2 || $filter_position==3}
            <div class="dropdown_wrap facet_dropdown_item">
              ...
              <div class="dropdown_list" aria-labelledby="{$facet.label}">
                  ...
                  {if in_array($facet.widgetType, ['radio', 'checkbox', 'rangeslider'])}
                    ...
                    {else}
                    <ul id="facet_{$_expand_id}" class="facet_filter_box collapse{if !$_collapse} in{/if} facet_{$facet.type}">
                      {include file='catalog/_partials/facets-input.tpl'}
                    </ul>
                    {/if}
                    {/block}
                  {elseif $facet.widgetType == 'slider'}
                    <div id="facet_{$_expand_id}" class="facet_filter_box collapse{if !$_collapse} in{/if} facet_{$facet.type}">
                      {include file='catalog/_partials/facets-slider.tpl'}
                    </div>
                  {else}
