...
{block name='product_list_header'}
    ...
    {if ($sttheme.display_category_image || (isset($steasybuilder) && isset($category.image.bySize.category_default.url) && $steasybuilder.is_editing)) && $category.image && $category.image.bySize.category_default.url}
        ...
        <div class="col-lg-4 category-cover {if !$sttheme.display_category_image} display_none {/if}">
...
{block name='product_list_footer'}
    {if ($sttheme.display_cate_desc_full==2 || (isset($steasybuilder) && $steasybuilder.is_editing)) && $category.description}
        <div id="category-description-bottom"
             class="category-description style_content mb-3 truncate_block st_showless_block_{if !empty($sttheme.showless_cate_desc)}1{else}0{/if} truncate_cate_desc_{$sttheme.truncate_cate_desc} {if $sttheme.display_cate_desc_full!=2} display_none {/if}">{if $sttheme.display_cate_desc_full==2}
            <div class="st_read_more_box ">{$category.description nofilter}</div>{/if}<a href="javascript:;"
                                                                                        title="{l s='Read more' d='Shop.Theme.Panda'}"
                                                                                        class="st_read_more"
                                                                                        rel="nofollow"><span
