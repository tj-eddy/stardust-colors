...
<div class="swiper-container products_sldier_swiper {if $direction_nav>1 && $direction_nav<10} swiper-button-lr {if $direction_nav==6 || $direction_nav==7} swiper-navigation-circle {elseif $direction_nav==4 || $direction_nav==5} swiper-navigation-rectangle {elseif $direction_nav==8 || $direction_nav==9} swiper-navigation-arrow {elseif $direction_nav==2 || $direction_nav==3} swiper-navigation-full {/if} {if $direction_nav==2 || $direction_nav==4 || $direction_nav==6|| $direction_nav==8} swiper-navigation_visible {/if}{/if} {if $direction_nav>=10} swiper-button-bottom {/if} swiper_loading" {if $sttheme.is_rtl} dir="rtl" {/if}>
	...
	{if $direction_nav>1 && (!isset($column_slider) || !$column_slider)}
		{if $direction_nav>=10}<div class="swiper-button-bottom-con flex_container {if $direction_nav==10} flex_center {elseif $direction_nav==11} flex_left {elseif $direction_nav==12} flex_right {/if}">{/if}
        ...
        {if $direction_nav>=10}</div>{/if}
    {/if}
    {if isset($scrollbar_nav) && $scrollbar_nav}<div class="placeholder-swiper-scrollbar"></div><div class="swiper-scrollbar"></div>{/if}
</div>
{if $control_nav}
